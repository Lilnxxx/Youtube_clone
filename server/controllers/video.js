...
export const addVideo=async(req,res,next)=>{
 ...
 export const deleteVideo=async(req,res,next)=>{
    try {
        ...
        if(req.user.id=== video.userId){
            ...
            return next(createError(403,"not allowed to delete video"))
        }
    } catch (error) {
        next(error)
    }
 ...
 export const sub=async(req,res,next)=>{
    try {
        ...
        const subschannel= user.subscribedUsers
        const list = await Promise.all(
            subschannel.map((channelId)=>{
                // console.log( Video.find({userId:channelId}))
